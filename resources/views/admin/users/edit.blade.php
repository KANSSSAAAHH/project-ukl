...
<title>Edit Review – PawonLokal</title>
...
<link rel="stylesheet" href="{{ asset('css/admin-review/edit.css') }}">
</head>
<body>
<div class="wrapper">
<aside class="sidebar">
...
<div class="main">
    <div class="topbar">
        <div>
            <div class="topbar-title">Edit Review</div>
    ...
    <div class="content">
        <div class="page-header">
            <h1>Edit Review</h1>
            <p>Ubah data review #{{ $review->id_review }}</p>
        </div>
        <div class="form-card">
            <form action="{{ route('admin.review.update', $review->id_review) }}" method="POST">
                @csrf @method('PUT')
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <select name="id_user" class="form-control" required>
                            ...
                            @foreach($users as $u)
                                <option value="{{ $u->id_user }}" {{ old('id_user', $review->id_user) == $u->id_user ? 'selected' : '' }}>
                                    {{ $u->nama }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <select name="id_produk" class="form-control" required>
                            ...
                            @foreach($produk as $p)
                                <option value="{{ $p->id_produk }}" {{ old('id_produk', $review->id_produk) == $p->id_produk ? 'selected' : '' }}>
                                    {{ $p->nama_produk }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="form-group">
                    ...
                    <div class="rating-hint">Rating saat ini: {{ $review->rating }}/5</div>
                    @error('rating') <div class="error-msg">{{ $message }}</div> @enderror
                </div>

                <div class="form-group">
                    <label>Komentar <span>*</span></label>
                    <textarea name="komentar" class="form-control" id="komentarInput" required>{{ old('komentar', $review->komentar) }}</textarea>
...
<script src="{{ asset('js/admin-review/edit.js') }}"></script>
</body>
</html>
...
:root {
    ...
    --crimson-pale: rgba(139,26,26,0.08); --gold: #C9923A; --gold-light: #E8B86D;
...
<div class="main">
    <div class="topbar">
        <div>
            ...
            <div class="topbar-breadcrumb">Admin <i class="fa-solid fa-chevron-right" style="font-size:.55rem"></i> <span>Pesanan</span></div>
        </div>
    </div>
            ...
            <div class="table-wrap">
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{ $i + 1 }}</td>
                            <td style="font-family:monospace;font-size:0.75rem;color:#aaa">{{ $p->id_pesanan }}</td>
                            <td>{{ $p->user->nama ?? '-' }}</td>
                            <td>{{ \Carbon\Carbon::parse($p->tanggal_pesanan)->format('d M Y') }}</td>
                            <td class="harga">Rp {{ number_format($p->total_harga, 0, ',', '.') }}</td>
                            <td><span class="badge badge-{{ $p->status_pesanan }}">{{ $p->status_pesanan }}</span></td>
                            <td>
                                <div class="action-group">
                                    <a href="{{ route('admin.pesanan.edit', $p->id_pesanan) }}" class="btn-edit">
                                        <i class="fa-solid fa-pen"></i> Edit
                                    </a>
                                    <form action="{{ route('admin.pesanan.destroy', $p->id_pesanan) }}" method="POST" onsubmit="return confirm('Hapus pesanan ini?')">
                                        @csrf @method('DELETE')

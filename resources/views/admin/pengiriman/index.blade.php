...
.search-box { display: flex; align-items: center; gap: 8px; background: var(--white); border: 1.5px solid #e8d8c4; border-radius: 10px; padding: 9px 14px; width: 280px; transition: border-color var(--tr), box-shadow var(--tr); }
...
.kota-badge { display: inline-flex; align-items: center; gap: 5px; background: #fef3c7; color: #92400e; padding: 3px 10px; border-radius: 50px; font-size: 0.72rem; font-weight: 600; }
...
(function () {
    ...
    function runFilter() {
        ...
        rows.forEach(row => {
            const matchQ    = !q    || row.dataset.name.includes(q)
                                    ...
                                    || row.dataset.kota.includes(q);
            const matchKota = !kota || row.dataset.kota === kota;
            const show      = matchQ && matchKota;
        ...
        if (resultCount) {
            resultCount.innerHTML = rows.length
                ? `Menampilkan <strong>${vis}</strong> dari <strong>${rows.length}</strong> pengiriman`
                : '';
        }
    ...
    filterKota && filterKota.addEventListener('change', runFilter);

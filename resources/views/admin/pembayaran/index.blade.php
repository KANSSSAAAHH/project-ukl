...
.order-id { font-family: monospace; font-size: 0.75rem; color: #aaa; }
...
/* ── METODE BADGE ── */
.metode-badge { display: inline-flex; align-items: center; gap: 5px; background: #f3f4f6; color: #374151; padding: 3px 10px; border-radius: 50px; font-size: 0.72rem; font-weight: 600; }

/* ── BUKTI BAYAR ── */
.bukti-img { width: 44px; height: 44px; border-radius: 8px; object-fit: cover; border: 2px solid #f0e8de; cursor: pointer; transition: transform var(--tr); }
.bukti-img:hover { transform: scale(1.08); }
.no-bukti { font-size: 0.75rem; color: #aaa; font-style: italic; }
...
.lightbox img { max-width: 90vw; max-height: 85vh; border-radius: 12px; box-shadow: 0 8px 40px rgba(0,0,0,0.4); }
.lightbox-close { position: absolute; top: 20px; right: 24px; background: none; border: none; color: #fff; font-size: 1.8rem; cursor: pointer; line-height: 1; }
...
(function () {
    ...
    function runFilter() {
        ...
        rows.forEach(row => {
            const matchQ      = !q      || row.dataset.id.includes(q) || row.dataset.metode.includes(q);
            const matchStatus = !status || row.dataset.status === status;
            const matchMetode = !metode || row.dataset.metode === metode;
            const show        = matchQ && matchStatus && matchMetode;
        ...
        if (resultCount) {
            resultCount.innerHTML = rows.length
                ? `Menampilkan <strong>${vis}</strong> dari <strong>${rows.length}</strong> pembayaran`
                : '';
        }
    ...
    filterMetode && filterMetode.addEventListener('change', runFilter);

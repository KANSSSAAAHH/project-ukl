        ...
        <div class="form-card">
            <form action="{{ route('admin.review.update', $review->id_review) }}" method="POST">
                ...
                <div class="form-group">
                    ...
                    <div style="font-size:0.75rem;color:var(--text-light);margin-top:6px">Rating saat ini: {{ $review->rating }}/5</div>
                    @error('rating') <div class="error-msg">{{ $message }}</div> @enderror
                </div>

                <div class="form-group">
                    <label>Komentar <span>*</span></label>
                    <textarea name="komentar" class="form-control" required>{{ old('komentar', $review->komentar) }}</textarea>

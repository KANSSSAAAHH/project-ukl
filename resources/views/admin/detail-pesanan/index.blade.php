...
/* ID PESANAN BADGE */
.id-badge { display: inline-flex; align-items: center; gap: 5px; background: #fef3c7; color: #92400e; border: 1px solid #fde68a; border-radius: 7px; padding: 3px 9px; font-size: 0.75rem; font-weight: 700; font-family: monospace; }
...
(function () {
    ...
    function runFilter() {
        ...
        const pesanan  = filterPesanan.value;
        const subtotal = filterSubtotal.value;
        ...
        rows.forEach(row => {
            ...
            const rSubtotal = parseInt(row.dataset.subtotal);

            const matchQ        = !q       || rProduk.includes(q) || rPesanan.includes(q);
            const matchPesanan  = !pesanan || rPesanan === pesanan;
            let   matchSubtotal = true;
            if (subtotal === '0-50000')       matchSubtotal = rSubtotal < 50000;
            if (subtotal === '50000-200000')  matchSubtotal = rSubtotal >= 50000 && rSubtotal <= 200000;
            if (subtotal === '200000+')       matchSubtotal = rSubtotal > 200000;

            const show = matchQ && matchPesanan && matchSubtotal;
        ...
        if (resultCount) {
            resultCount.innerHTML = rows.length
                ? `Menampilkan <strong>${vis}</strong> dari <strong>${rows.length}</strong> detail`
                : '';
        }
    ...
    filterPesanan  && filterPesanan.addEventListener('change', runFilter);
    filterSubtotal && filterSubtotal.addEventListener('change', runFilter);

...
.login-left img { width: 100%; height: 100%; object-fit: cover; object-position: center; display: block; transition: transform 0.6s ease; }
.login-left:hover img { transform: scale(1.03); }
.login-left-overlay {
    position: absolute; inset: 0;
    background: linear-gradient(to right, rgba(92,13,13,0.3) 0%, rgba(92,13,13,0.1) 60%, transparent 100%);
    pointer-events: none;
}
.login-left-text { position: absolute; bottom: 40px; left: 36px; z-index: 2; }
.login-left-text h2 {
    font-family: 'Playfair Display', serif;
    font-size: 2rem; font-weight: 800; color: var(--white);
    text-shadow: 0 2px 16px rgba(0,0,0,0.4); line-height: 1.2; margin-bottom: 8px;
}
.login-left-text p { font-size: 0.85rem; color: rgba(255,255,255,0.8); text-shadow: 0 1px 8px rgba(0,0,0,0.3); }

/* DIVIDER */
.login-divider { width: 4px; background: linear-gradient(to bottom, var(--crimson-deep), var(--crimson), var(--gold)); flex-shrink: 0; }

/* KANAN — FORM */
.login-right {
    width: 440px; flex-shrink: 0; background: var(--white);
    display: flex; flex-direction: column; align-items: center; justify-content: center;
    padding: 32px 36px; overflow-y: auto; position: relative;
}
.login-right::before {
    content: ''; position: absolute; top: 0; left: 0; right: 0;
    height: 4px; background: linear-gradient(to right, var(--crimson), var(--gold));
}

.login-logo { text-align: center; margin-bottom: 20px; width: 100%; }
.login-logo img { width: 64px; height: 64px; object-fit: contain; margin-bottom: 8px; filter: drop-shadow(0 4px 12px rgba(139,26,26,0.2)); }
.login-logo-title { font-size: 1.3rem; font-weight: 800; color: var(--text-dark); letter-spacing: 0.05em; text-transform: uppercase; }
.login-logo-sub { font-size: 0.75rem; color: var(--text-light); margin-top: 3px; }

/* Alert */
.alert-error { background: #fee2e2; border: 1px solid #fecaca; color: #b91c1c; padding: 10px 14px; border-radius: 10px; font-size: 0.8rem; font-weight: 600; margin-bottom: 14px; display: flex; align-items: center; gap: 8px; width: 100%; }
...
.input-wrap i { position: absolute; left: 14px; top: 50%; transform: translateY(-50%); color: var(--text-light); font-size: 0.85rem; }
.form-control {
...
.error-msg { font-size: 0.73rem; color: var(--crimson); margin-top: 3px; font-weight: 600; }
...
.btn-register:hover { transform: translateY(-2px); box-shadow: 0 8px 28px rgba(139,26,26,0.45); }

.login-link { text-align: center; font-size: 0.82rem; color: var(--text-mid); font-weight: 600; }
.login-link a { color: var(--crimson); font-weight: 700; text-decoration: none; }

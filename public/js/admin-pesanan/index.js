(function () {
    ...
    function runFilter() {
        ...
        rows.forEach(row => {
            ...
            // Match conditions
            const matchQuery  = !query || rowName.includes(query) || rowId.includes(query);
        ...
        if (resultCount) {
            resultCount.innerHTML = rows.length
                ? `Menampilkan <strong>${visibleCount}</strong> dari <strong>${rows.length}</strong> pesanan`
                : '';
        }
    ...
    console.log('✅ Index Pesanan loaded with filter functionality');

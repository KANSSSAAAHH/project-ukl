(function () {
    ...
    function runFilter() {
        ...
        rows.forEach(row => {
            ...
            const matchQuery = !query || rowName.includes(query) 
                                      ...
                                      || rowKota.includes(query);
            const matchKota  = !kota || rowKota === kota;
            const shouldShow = matchQuery && matchKota;
        ...
        if (resultCount) {
            resultCount.innerHTML = rows.length
                ? `Menampilkan <strong>${visibleCount}</strong> dari <strong>${rows.length}</strong> pengiriman`
                : '';
        }
    ...
    if (filterKota) {
        filterKota.addEventListener('change', runFilter);
    }
    ...
    console.log('✅ Index Pengiriman loaded with filter functionality');
